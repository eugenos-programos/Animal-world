...
class Test(unittest.TestCase):
    ...
    def tests_for_herbivore_and_predator_classes(self) -> None:
        print(Fore.WHITE + "Herbivore and predator classes testing",
              end='',
              flush=True)
    ...
    def cell_tests(self) -> None:
        ...
        animals_for_cell = [Rabbit(Sex.MALE), Lion(Sex.MALE)]
        plant_on_cell = Plant()
        ...
        self.assertListEqual([cell.get_column_index(),
                              cell.get_row_index()], [3, 2])
        pprint()
        self.assertEqual(cell.get_plant_on_cell(), plant_on_cell)
        correct_information = [
            'P--1(4)      ', 'R--1(mal,4,4)', 'L--1(mal,5,4)', 'Z--1(fem,4,4)'
        ]
        self.assertEqual(correct_information, cell.info())
        cell.next_step()
        correct_information = [
            'P--1(3)      ', 'R--1(mal,3,3)', 'L--1(mal,4,3)', 'Z--1(fem,3,3)'
        ]
        pprint()
        self.assertEqual(correct_information, cell.info())
        for iter in range(3):
    ...
    def tests_for_area_class_1(self):
        ...
        cell1 = Cell(
            ...
            0,
            Plant(),
            [  ### id for animals will be set auto
            ])
        cell2 = Cell(0, 1, animals=[Rabbit(Sex.MALE), Rabbit(Sex.FEMALE)])
        cell3 = Cell(1, 0, animals=[Lion(Sex.MALE), Lion(Sex.FEMALE)])
        ...
        self.assertSetEqual(set(area._Area__get_neighbor_cells(cell1, 1)),
                            {cell2, cell3, cell4})
        pprint()
        self.assertSetEqual(set(area._Area__get_neighbor_cells(cell2, 2)),
                            {cell1, cell3, cell4})
        self.assertSetEqual(set(area._Area__get_neighbor_cells(cell3, 3)),
                            {cell1, cell2, cell4})
        pprint()
